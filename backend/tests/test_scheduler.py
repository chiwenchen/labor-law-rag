...
async def test_run_law_update_success_writes_log():
    """Successful update writes a 'success' log entry."""
    ...
    with patch("app.services.scheduler.fetch_labor_law_articles", return_value=mock_articles), \
...
async def test_run_law_update_failure_writes_error_log():
    ...
    with patch("app.services.scheduler.fetch_labor_law_articles",
